...
const DetailLession = ({ onBack, progress }) => {
  ...
  const handleCreateComment = async () => {
    ...
    try {
      const createCommentDto = {
        ...
        lessionId: video.id, // ID video hiện tại
      };
  ...
  const fetchComments = async () => {
    try {
      setLoading(true);
      const response = await requestApi.getRequest(`/comment/${lessonId}`); // Endpoint lấy tất cả comments
  ...
  useEffect(() => {
    const fetchVideo = async () => {
      try {
        const response = await requestApi(`/lessions/getVideo/${lessonId}`, 'GET');
